...
<div class="poll">
    ...
    <form method="post" action="mc1/vote">
        ...
        <c:if test="${not empty options[0]}">
            <c:forEach var="option" items="${options[0]}" varStatus="oStatus"> <!-- Loop through the options for the first question -->
                <div class="option">
                    <input type="radio" name="optionId" value="${oStatus.index}" id="opt_${oStatus.index}"> <!-- Use index for value -->
                    <label for="opt_${oStatus.index}">${option} <span>(${votes[0][oStatus.index]} votes)</span></label> <!-- Display vote count -->
                </div>
            </c:forEach>
        </c:if>
